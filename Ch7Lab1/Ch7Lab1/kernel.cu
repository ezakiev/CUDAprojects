...
// Ядро
__global__ void Prod(const BASE_TYPE* A, const BASE_TYPE* B, BASE_TYPE* C)
{
...
int main()
{
	...
	cudaMemcpy(dev_b, b, N * sizeof(BASE_TYPE), cudaMemcpyHostToDevice);

	int GRID_SIZE = N / BLOCK_SIZE + 1;
	Prod <<<GRID_SIZE, BLOCK_SIZE>>> (dev_a, dev_b, dev_c);
